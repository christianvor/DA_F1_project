...
For the regression tree we want to analyze the biggest inpacting factors on the Finishing position. For that we select the races taking place after 2000, as due to rule changes older races are hard to compare. Also we remove the Finishing Position 24, as these are mostly drivers that didn't finish the race and thus cause a high correlation. It is also importatn to note, that no points are included in the dataset, as Points and Finishing positions are highly correlated (Finishin well = Getting lots of Points).
...
plot(as.party(tree))
```
We create an unpruned tree on all the Variables of the Dataframe, and predict the finishing postion. It can be seen that the tree is very messy, but a few key points can be identivied. 1. The Tree splits first into top 10 starting postion and bottom 10 starting postions. For the Top the startes he then splits again for starting postions bevor considering the the Race taking place. For the bottom 10 startes the contstructor ID is the next splitting point. Followed by splitting for driver or Race name. 

From this simple tree we can concluded that starting postions as well as the Race have an high impact into the finishing postion. When comparing this to the things we know from the actual races, this make sense as starting in a good postion has an high impact on finishing well, especally on tracks where it is hard to overtake. 
...
```
We plot the Error of the tree compared to its number of splits. Because of the big data set with multipl variables the plot is very hard to read. In order to make it more readible we zoom in. 
...
When focusing on a tree size between 1 - 15 we can identify a sice of 5 as the best pick for reducing the error as well as keeping the tree simple. 
...
```
We prune the tree to a size of 5. Now it is easier to understand the final splits of the tree. For   
