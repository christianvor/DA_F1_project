...
We start off by explaining which data points exactly we are taking a look at and why those are of interest to us. Next we go through the different models we employed in the following order: Lin Reg, Logit, Trees and Forests, Clustering. After that we conclude our research and name future improvements and further topics that can be looked into. There are multiple instance at which we explain how we cleaned the data, since the different methods demanded us to apply different approaches, this is not the result of laziness, but clarity for reproducing our results.  
...
### Motivation and Our Research Question

In this project, we tried to explore the main impacting factors for winning a race in Formula 1. To this end, we used different data analytics methods, such as linear regression, logistic regression, decision trees, random forest, and clustering. 

We want to bring value to stakeholders by proving the significant impact of qualifying results on the race result(Regressions), estimating and forecasting the future performance of drivers and teams(Decision Trees and Random Forest), and visualizing the dominance of certain teams through the years(Clustering). 

Keeping all these in mind, the research question we're trying to answer in our analysis is: " Is it possible to predict F1 races and know the performance of a driver in the next races and year?"


### Exploring the data

We have noticed that the main NAs we have in the whole data are the ones resulting from drivers not finishing the race. We have replaced these NAs with "24" for the FinishingPosition variable as this is the lowest possible finishing position. 

For the purpose of our project, we focus on data from all the sessions on one race weekend, paying special attention to the variable, 'FinishingPosition' as it gives us the most insight to answer our research questions.

We start our analysis with the pre-processed data. First of all, we eliminate the NAs and Null the variable 'X' as it just gives us index of the rows and is redundant. 
...
As we can see, we have both numerical and categorical variables in the data set. The categorical variables include the variables, 'Country', 'officialName', 'driverId', 'constructorId', and 'engineManufacturerId', while the numerical variables are 'raceId', 'year', 'round', 'PositionPractice1', 'PositionPractice2', 'StartingPosition', 'FinishingPosition', and 'points'.

Next, we can also take a look at the correlation of the numeric variables to have the first impression of the variables and their potential relations:
...
F1_complete$X <- NULL # Rm X because not relevant
F1 <- F1_complete[,1:12] # Select Races after 2000
F1_new <- na.omit(F1) # Rm Na 
F1 <- F1_new[which(F1_new$FinishingPosition != 24),]# Remove points because of high correlation to Finishing 
```

We want to analyze the regression tree's most impacting factors on the Finishing position. We select the races after 2000, as older races are hard to compare due to rule changes. Also, we removed the Finishing Position 24, as most drivers didn't finish the race, thus causing a high correlation. It is also important to note that no points are included in the dataset, as Points and Finishing positions are highly correlated (finishing well = Getting lots of Points).
...
plot(as.party(tree))
```
We create an unpruned tree on all the Variables of the data frame and predict the finishing position. The tree is very messy, but a few key points can be identified. 1. The Tree splits first into the top 10 starting positions and the bottom ten starting positions. For the top, it splits again for starting positions before considering the Race taking place. The constructor ID is the next splitting point for the bottom ten states, followed by splitting for the driver or Race name. 

From this simple tree, starting positions and the race name significantly impact finishing positions. This makes sense when comparing this to what we know from the actual races, as beginning in a good position significantly impacts finishing well, especially on tracks that are hard to overtake. 

### Finding the best number of splits for the tree
```{r}
...
```
We plot the tree's error compared to its number of splits. The plot is tough to read because it contains a big data set with multiple variables. To make it more readable, we zoom in. 
...
When focusing on a tree size between 1 and 15, we can identify a slice of 5 as the best choice for reducing the error and keeping the tree simple. 
...
```
We prune the tree to a size of 5. Now, it is easier to understand the final splits of the tree. For the top 10 drivers, we see a second split for the top 5 drivers and the other top 10 drivers. We see a clear advantage for starting in the top 5; the boxplot shows that the mean finishing position is around the 4th place, whereas for a starting position between 5 and 10, the mean finishing position is around the 6th place. For drivers with a starting position>10, the constructor ID splits between good and bad teams. For the good teams, there is another split for drivers, signaling that driver quality is only a decisive factor for good teams. A good driver starting from a place >10 but with a good team has a mean finishing position of 10, whereas a lousy driver starting in the same circumstance will finish around P14. For the rest, the finishing position is around P16.



### Makeing predictions with the pruned tree
```{r}
raceId <- 1102
year <- 2024
round <- 1 
...
```
We use our pruned tree to predict how well Charles Leclerc performs on the first race in the season. This is an insample prediction as the race is represented in our Dataset. We can see that our tree predicts an outcome of P3 or P4. When comparing this to the actual race where Leclerc finished P4, the prediction is pretty close. 
