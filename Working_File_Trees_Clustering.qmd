...
```
The first table shows us how many times each team has been in each cluster, giving us a feel for how much their performance varies over time.
In the second table we can see how many different teams have been in the different clusters which is an interesting visual since we can see that only 4 teams managed to be in the top cluster over the last 13 years, but also only 5 different teams have been in the lowest cluster over this time. From this we can conclude that good teams stay good most of the times and poor performing teams have a hard time scoring better.
